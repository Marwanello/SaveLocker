...
### Server (`src/Server`)
- `Program.cs` — minimal-API endpoints + DI. Schema via `db.Database.Migrate()` on startup; a bootstrap shim pre-stamps `InitialSchema` on existing DBs so `Migrate()` skips it. Two auth filter groups: `ApiKeyFilter` (agent routes, `X-Api-Key`) and `AdminPasswordFilter` (dashboard routes, when a password is set: a console session `X-Admin-Session`, or the password itself in `X-Admin-Password` for scripts). Also wires the opt-in forwarded-client-address handling (`Security:TrustedProxies`) and the response-header/CSP middleware.
...
- `Services/AdminAuth.cs` — the console's admin credential: throttled password checks (with a short-lived cache of *successful* verifications so scripts do not pay PBKDF2 per request), and revocable sessions (`AdminSession` — random token, SHA-256 stored, sliding expiry). `Services/AuthThrottle.cs` — in-memory per-client lockout + global backstop for admin-password guesses. See `Decisions.md`.
