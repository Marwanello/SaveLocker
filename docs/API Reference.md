...
- **Admin routes** — dashboard / human operations. When no admin password is set every admin route is open. Otherwise a request carries **either** a console session, `X-Admin-Session: <token>` (minted by `POST /api/admin/session`, revocable, expires on its own — what the dashboard uses), **or** the password itself, `X-Admin-Password: <password>` (for scripts). Check `GET /api/admin/status` to know if a password is required.
- **Password attempts are throttled** (`Security:*`, see *Securing the server* in [[Build and Run]]): after `MaxFailedAttempts` (5) wrong passwords inside 15 minutes a client is refused with **429** + `Retry-After` — even for the right password — on every path that checks one: `POST /api/admin/session`, the `X-Admin-Password` header, and `POST /api/machines/register`. A wrong or expired **session token** is a plain **401** and is never counted.
...
## Public (no auth)
- `POST /api/machines/register` `{ name }` → `{ machineId, apiKey }`. First-time registration of a new name is open. Re-registering an **existing** name rotates its key — requires `X-Admin-Password` once an admin password is set (**401** otherwise; **429** while throttled). With no password configured it stays open. `Security:RequireAdminPasswordToRegister=true` makes a password required for **new** names too (a machine key is fleet-scoped — [[Decisions]] §4); off by default.
- `POST /api/admin/session` `{ password }` → `{ token, expiresAt }` — **sign in**: exchange the admin password for a revocable session token, returned once (only its SHA-256 is stored). `token` is `null` when the server has no admin password. **401** for a wrong password, **429** + `Retry-After` while throttled. Idle expiry slides 7 days (`Security:SessionIdleDays`) up to a 30-day cap (`Security:SessionMaxDays`).
...
- `GET /api/games/{id}/art/options?kind=grid|icon&page=0` → `ArtOptionsPageDto { kind, page, hasMore, options[] }`, five options per page (`page` is 0-based; past the end — including an absurd page number, which is answered without asking SteamGridDB — is an empty page, not an error). Each `ArtOptionDto { url, preview, width, height, author }`: `url` is what `PUT` below takes; `preview` is a small **inline `data:` image** the server fetched and shrunk (the console's CSP allows nothing else, and this keeps the browser off SteamGridDB's CDN). 400 `{ message }` for an unknown kind/game, no key, no SteamGridDB match, or SteamGridDB unreachable. The listing is kept in memory for 10 minutes, so paging costs no extra SteamGridDB requests.
- `PUT /api/games/{id}/art/{kind}` (`kind` = `grid` or `icon`) body `{ url }` → the updated `GameDto`. Downloads and stores that image as the game's cover/icon. The URL goes through the same checks as every art fetch (https, allowlisted host on every redirect hop, size cap, pixel cap, type decided by the bytes): a URL off the allowlist is 400 and never contacted. Needs no API key — only *listing* does.
- `GET /art/{gameId}/{grid|icon}.{ext}?w={n}` (static, no auth, like the rest of `/art`) → a cached copy resampled to `n` px wide (Lanczos, aspect kept, never upscaled). `n` must be one of **48, 64, 96, 128, 192, 256, 384**; any other value, or no `w`, serves the untouched original. Opaque covers come back as JPEG, icons as PNG. Made on first request into `{gameId}/thumbs/`; a re-fetched image invalidates it by being newer.
...
- `POST /api/games/{id}/excludes` body `string[]` → 200 / 400 / 404. Per-game exclude globs. Empty array clears them back to `Sync:DefaultExcludeGlobs`. **400** (with the reason) for a pattern the matcher cannot evaluate (a `..` anywhere but the start), a line break inside a pattern, more than 100 patterns, or one over 260 characters — nothing is stored.
- `POST /api/games/{id}/excludes/preview` body `string[]` → `{ wouldExclude }` / 400 / 404. Dry run of a **draft** pattern list against the game's head archive (the archive's own zip index, via the same matcher the agent uses); never stores anything. Counts the whole draft, so it can include files a *saved* pattern already covers when the head predates that pattern. 0 for a game with no head. Same 400 rules as saving.
...
## Server settings
- `GET /api/settings` → `ServerSettingsDto { steamGridDbConfigured, steamGridDbKeyMasked, steamGridDbFromConfig, adminPasswordSet, defaultExcludeGlobs, autoFetchHours, schedule, nextAutoFetchRunAt, appearance }`. Never returns the raw key. `appearance` is `AppearanceSettingsDto { look: { theme, accent, mark }, pushToAgents }` — always known-good ids, whatever is stored or set in the environment.
- `POST /api/settings/appearance` body `{ theme, accent, mark, pushToAgents }` → `AppearanceSettingsDto`. *(admin)* The console's look (checkpoint-ui Phase 4). `theme` ∈ `system|dark|light`, `accent` ∈ `ember|coolant|arcade|cobalt|emerald|stealth`, `mark` ∈ `pixel|cartridge|memcard` (`Appearances` in `src/Shared/Appearance.cs`; case-insensitive, stored lower-case); anything else is a **400** and nothing is stored. Ids, never colours: the palette each id means lives in the front ends. Audited as `settings.appearance`. Stored as `Ui:Theme`/`Ui:Accent`/`Ui:Mark`/`Ui:PushToAgents` (an absent `PushToAgents` reads **true**), and each is also settable from configuration (`Ui__Accent=coolant`), the DB value winning.
- `POST /api/settings/steamgriddb-key` body `{ apiKey }` → `{ ok, message, gamesQueued }`. **Verifies, then stores** — a key that fails verification answers **400** `{ ok: false, message, keptExistingKey }` and leaves the existing key exactly as it was. The probe hits an authenticated endpoint (`grids/game/{id}`); `search/autocomplete` is public and would accept anything. Null/blank clears the DB override (falls back to config) without a verification round-trip.
  On success the answer also carries `gamesQueued`: games with no cover or no icon are filled in **in the background** (`ArtBackfillService`, one at a time) and the message says so. Only the *missing* art is fetched — a cover chosen by hand is never replaced. The same pass also runs once shortly after every server start (`Art:BackfillOnStartup`, default true; `Art:BackfillStartupDelaySeconds`, default 30), so a key supplied by configuration (`SteamGridDb__ApiKey`), which is never "saved" here, backfills too, and a restart mid-run loses nothing.
...
- `POST /api/admin/password` `{ password }` → `{ ok, message }`. Set or clear the admin password. **Ends every console session**, the caller's included (the console signs straight back in). New passwords are stored as `v2:600000:` PBKDF2; an older `v1:` hash is re-written at the next successful sign-in.
- `DELETE /api/admin/session` → 204. **Lock**: ends the session named by `X-Admin-Session`. Idempotent; a no-op for a caller that used the password header.
- `DELETE /api/admin/sessions` → `{ ended }`. **Sign out everywhere**: ends every session, this one included.
...
- `POST /api/agent/health` body `AgentHeartbeat { agentVersion, platform, lastSyncTime?, trackedGames, unmappedGames, offlineQueueDepth, events?, resolvedGameIds? }` → `AgentHeartbeatResponse { escalatedConflicts, appearance? }`. Piggybacks the existing ~20 s poll, so it adds no schedule. `appearance` is the console's look (`{ theme, accent, mark }`) **when pushing is on, and absent when it is off** — an agent reads absent as "no opinion" and keeps what it last applied. Optional/additive, so an older agent ignores it and a newer agent talking to an older server sees none. **This is the only way a headless agent can tell anyone anything** (`Decisions.md` §2 — the console is the Deck's UI). A conflict open for more than six hours is returned so a Windows tray agent can notify the user even when the stuck machine is a Deck.
...
**Every route in this section takes an optional `?platform=`.** Absent means `win-x64`, so a console or script written before platform slots existed keeps managing the Windows installer unchanged. Valid values are `win-x64`, `linux-x64` and `decky-plugin` (`AgentPlatform.All`); anything else is **400** rather than defaulted — `?platform=` selects a directory, and a caller asking for a platform this server does not host wants to hear so, not to be handed the Windows installer instead. Storage is `{root}` for `win-x64` and a subdirectory named for the platform beside it, so no deployed server needed migrating.

**`decky-plugin` is not an agent**, and rides these routes deliberately. It is the Decky plugin the *Linux agent* installs into `~/homebrew/plugins/SaveLocker` on the user's behalf, and a slot is exactly what it needs — an upload, a digest, an atomic replace and a version to compare — so it gets one rather than a near-identical second set of routes. It is also the only slot whose GitHub asset comes from a **different repository** (`AgentUpdate:Plugin:GitHubRepo`, default `SkorcherX/SaveLocker-Decky`), which is why a slot carries its own repo.
...
- `GET /api/agent/latest?platform={p}` → `AgentVersionInfo { latestVersion, downloadUrl, sha256 }`, or 204 if no installer is configured for that platform. Checks the filesystem first (`AgentInstallerService`); falls back to static `AgentUpdate:LatestVersion` + `DownloadUrl` config, with the Linux fallback under `AgentUpdate:Linux:*` and the plugin's under `AgentUpdate:Plugin:*` (env `AgentUpdate__Linux__DownloadUrl` — deliberately not a dashed section name, because `AgentUpdate__linux-x64__…` is not a legal bash identifier and the Linux harness has to export it).
...
- `POST /api/commands` `{ machineId, gameId?, type, force }` → `AgentCommandDto` (dashboard queues a command; `type` = `Pull|Push|Sync|Scan`). **400** for an unknown machine, game or type. *(admin)*
- `POST /api/commands/bulk` `{ commands: [{ machineId, gameId?, type, force }], skipMachinesUnseenForSeconds? }` → `{ queued: AgentCommandDto[], skipped: [{ machineId, machineName, reason }] }` — the console's **Sync all** (one `Sync` per machine, `gameId: null` = everything that machine tracks). **All-or-nothing**: every request is validated first (**400** naming the unknown id) and everything is saved in one commit; at most 100 per call. A request identical to a command already **Pending** is answered with *that* command rather than stacking a second one. Commands never expire, so `skipMachinesUnseenForSeconds` (the console sends 180) leaves out machines that have not contacted the server that recently — otherwise a switched-off machine would fire the sync, unannounced, whenever it next connects. *(admin)*
