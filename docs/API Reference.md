...
- `GET /api/games/{id}/art/options?kind=grid|icon&page=0` → `ArtOptionsPageDto { kind, page, hasMore, options[] }`, five options per page (`page` is 0-based; past the end is an empty page, not an error). Each `ArtOptionDto { url, preview, width, height, author }`: `url` is what `PUT` below takes; `preview` is a small **inline `data:` image** the server fetched and shrunk (the console's CSP allows nothing else, and this keeps the browser off SteamGridDB's CDN). 400 `{ message }` for an unknown kind/game, no key, no SteamGridDB match, or SteamGridDB unreachable. The listing is kept in memory for 10 minutes, so paging costs no extra SteamGridDB requests.
- `PUT /api/games/{id}/art/{kind}` (`kind` = `grid` or `icon`) body `{ url }` → the updated `GameDto`. Downloads and stores that image as the game's cover/icon. The URL goes through the same checks as every art fetch (https, allowlisted host on every redirect hop, size cap, type decided by the bytes): a URL off the allowlist is 400 and never contacted. Needs no API key — only *listing* does.
...
- `POST /api/settings/steamgriddb-key` body `{ apiKey }` → `{ ok, message, gamesQueued }`. **Verifies, then stores** — a key that fails verification answers **400** `{ ok: false, message, keptExistingKey }` and leaves the existing key exactly as it was. The probe hits an authenticated endpoint (`grids/game/{id}`); `search/autocomplete` is public and would accept anything. Null/blank clears the DB override (falls back to config) without a verification round-trip.
  On success the answer also carries `gamesQueued`: games with no cover or no icon are filled in **in the background** (`ArtBackfillService`, one at a time) and the message says so. Only the *missing* art is fetched — a cover chosen by hand is never replaced.
