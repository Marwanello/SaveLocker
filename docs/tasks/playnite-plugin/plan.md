...
| 18 — Agent-side Playnite library reader + `agent-ui` tab (no plugin required) | ✅ Shipped 2026-09-16, re-verified via `testenv.ps1` 2026-09-17 — `src/Agent/PlayniteLibrary.cs` + `GameScanner` 4th source + `agent-ui` Playnite filter chip. Hardware-verified against this machine's real, installed Playnite — which caught and fixed two real bugs the risk section below anticipated but got wrong in the first pass: the real `games.db` is LiteDB 4's on-disk format, not 5's, and the collection is named `Game`, not `games`. A THIRD real bug found re-verifying through `testenv.ps1` against the project's own portable `Playnite-Test` instance: a portable install keeps its library beside its own exe, not `%AppData%`, so this whole source was silently invisible to it — fixed with a `SAVELOCKER_PLAYNITE_PATH` override `testenv.ps1 -PlaynitePath` now sets automatically. 11/11 new unit tests passing |
| 19 — `agent-ui` "Playnite plugin" card: suggest by default, one-click direct install as an explicit opt-in | ✅ Shipped 2026-09-16, re-verified via `testenv.ps1` 2026-09-17 — `PlaynitePluginCard.tsx` + `GET /api/playnite-plugin/status` + `POST /api/playnite-plugin/install` (`PlaynitePlugin.InstallFirstTimeAsync`). First verified against a scratch dev daemon (not-installed state, refusal message); re-verified properly through `testenv.ps1` against the real portable `Playnite-Test` instance — installed the real v0.1.0 release build, the card correctly read back `INSTALLED v0.1.0`, and restarting Playnite loaded it cleanly (`ExtensionFactory:Loaded plugin: SaveLocker, version 0.1.0` in Playnite's own log) — the first confirmed real-Playnite load of a tagged release build |
