...
| 6 — Deck and Wayland | 🚧 Items 1-3 ✅ shipped 2026-09-22 (Group 6) — tokens, 62px rows, the button legend, Sync all on Y; verified live (real screenshots, pixel-sampled colours, `--nav`-scripted L1/R1) since `savelocker ui` runs on this Windows box without WSLg — a real focus-timing bug was found and fixed this way. Same-day follow-up fixed a mis-angled `Icons.Sync` arrowhead and swapped the header's stale pre-Checkpoint `logo-96.png` for a live `AppMark` drawn in the current accent/mark. Still no real Deck/gamescope pass; Archivo TTFs still needed for the font face. Item 4 (Wayland) still needs the open decision made first |
