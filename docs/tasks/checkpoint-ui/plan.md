...
## Status (updated 2026-09-23, after Group 6's follow-ups)
...
| 6 — Deck and Wayland | 🚧 Items 1-3 ✅ shipped 2026-09-22 (Group 6) — tokens, 62px rows, the button legend, Sync all on Y; verified live (real screenshots, pixel-sampled colours, `--nav`-scripted L1/R1) since `savelocker ui` runs on this Windows box without WSLg — a real focus-timing bug was found and fixed this way. Same-day follow-up swapped the header's stale pre-Checkpoint `logo-96.png` for a live `AppMark` drawn in the current accent/mark, and took two passes to fix `Icons.Sync`: the first pass's arrowhead-angle fix still overlapped into a blob at the icon's real ~18px size (a `--screenshot` render at a larger fixed size had made it look fine), caught only once the user tested it live — redrawn as one non-overlapping circle with two 140° arcs. Same round also fixed agent-ui's Games tab rendering game names in the system font instead of Archivo (`.sl-row`/`.sl-tile` render as `<button>`, which don't inherit font-family, and were missing the `font: inherit` every other interactive rule in `ui.css` already had). **2026-09-23:** stopped hand-guessing curved icon geometry entirely — a new `Ui/SvgPath.cs` strokes lucide's own `d` path strings (copied from `agent-ui/node_modules/lucide-react`), so `Icons.Cloud`, `Icons.Sync` and `Icons.GitBranch`'s arc now trace the real upstream shape instead of a third hand-eyeballed guess; also fixed the button-legend labels rendering visibly low against their glyph badges (`Widgets.HintLabel` was calling `AlignTextToFramePadding` against a box that was never framed). **Same day, later:** the user supplied the two Archivo TTFs — `Theme.cs` now embeds and bakes them in place of Inter, closing the last open item in Phase 1's "agent half" for this surface; Inter's files are gone from the repo. Still no real Deck/gamescope pass. Item 4 (Wayland) still needs the open decision made first |
