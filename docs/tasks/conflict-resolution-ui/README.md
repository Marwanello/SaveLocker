...
2026-08-30 from `docs/logs/` (via `SaveLocker/docs/logs/` after the vault reorg) and repo-root `docs/design/` — a stub at the old `logs/` path
points here). This folder is **not** an ordinary single-file task — it's a
...
**Once every phase in `plan.md` ships**, move this whole folder to `docs/logs/` with a date
