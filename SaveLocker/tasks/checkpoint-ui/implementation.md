...
Read [[plan]] first for tokens, type, motion and the colour rule.
...
5. Build the shared primitives the rest of the phases assume: `Card`, `Chip`, `Button` (primary /
   ...
   These are new files under `web/src/components/ui/`, and `agent-ui` imports them via a relative
   path or a small shared package — pick one and be consistent.
...
| Cover art in list and grid | Extend | `/art/{gameId}/{kind}.jpg` already serves; the grid needs the 600×900 `grid` kind fetched by `ArtService` alongside the existing kinds, and a fallback tile when SteamGridDB has nothing |
...
| **Games tab** | **New** | New view listing tracked games with cover art, grid or list, opening a per-game page: save size, last sync, versions, bytes sent last push, folder, process, launch command, Sync/Push/Pull. Data is already in `GET /api/games` + `GET /api/games/{id}/sync-status` |
...
- Phase 4 touches the wire format (heartbeat response) — regenerate `src/Server/openapi.json` and
  `web/src/api-types.ts` and commit both, per `CLAUDE.md`.
