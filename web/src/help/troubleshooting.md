...
- **"No tracked game matches this launch"** — the game has no `--appid`, so the `savelocker run` wrapper can't match it to a tracked game. Run `savelocker scan` to find the correct AppID, then re-add it: `savelocker add-game --name … --dir … --appid <id>`. (`--appid` is the one thing the agent UI's path browser can't set for you — the browser maps folders, not AppIDs.)
...
- **Game not in the Ludusavi manifest** — common for standalone builds, and not a failure. You map the save folder yourself, and on Linux that is the normal path rather than a fallback. **You do not have to type it:** browse to `http://localhost:5178` in Desktop Mode and use **Settings → Set save path**, which opens a D-pad-navigable folder browser; or, if this console shows `<machine>'s scan found: <path>` under the game's **Save paths per machine**, click **Apply**. From a terminal, `savelocker scan` offers to map anything it recognises, and `savelocker add-game --name … --dir <path>` is the manual route. See [Adding games](#help/adding-games).
