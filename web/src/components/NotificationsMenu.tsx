...
/** `AgentEventCodes.SaveDirMissing` — the other event worth deep-linking to a specific field for. */
...
export function NotificationsMenu({ problems, onOpenGame, onDismissProblem }: Props) {
  ...
  async function handleDismissAll() {
    if (!onDismissProblem) return;
    for (const id of dismissableIds) await onDismissProblem(id);
  }
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+8px)] w-[460px] bg-panel border border-line rounded-lg shadow-2xl z-30 overflow-hidden">
          <div className="px-3.5 py-2.5 border-b border-line flex justify-between items-center">
            ...
            {dismissableIds.length > 0 && onDismissProblem && (
              <button
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    {p.code === SAVEDIR_MISSING_CODE && p.gameId && (
                      <Button
                        ...
                        title="Open this game's folder field"
                      >
                        Open game
                      </Button>
                    )}
