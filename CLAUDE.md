...
The Obsidian vault is at `docs/` (repo root). Content lives under `docs/` — one folder per backlog item under `docs/tasks/` (`plan.md` or `summary.md` plus supporting files), write-ups under `docs/logs/`. Task folders are fed one file at a time per the Backlog line, not always as a single flat file.
